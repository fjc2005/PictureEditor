    ...
    ,delay(0)
{
    ...
    std::cout<<"succeed in load model!"<<std::endl;
...
void MainWindow::on_action_Open_triggered()
{
    ...
    if(srcDirPathListS.size()>=3){
        ...
        std::string path;
        QString2stdString(origin_path, path);
        img.load(path);
...
{
    QStringList srcDirPathListS = QFileDialog::getOpenFileNames(this, tr("choosePhoto"), "E:/Qt/qtworks/MainWindow/images", tr("图像文件(*.jpg *.png *.bmp)"));
    if(srcDirPathListS.size()>0)
    {
        ui->tabWidget->setCurrentIndex(0);
    }
...
void MainWindow::on_label_other_1_linkActivated(const QString &link)
{
    ;
}
...
void MainWindow::on_pushButton_save_img_sr_clicked()
{
    if(ui->label_show->pixmap()!=nullptr){
            ...
            {
                std::string path;
                QString2stdString(filename, path);
                cv::imwrite(path, sr_img);
            }
...
void MainWindow::on_pushButton_bic_clicked()
{
    if(ui->label_show->pixmap() != nullptr){
        cv::Mat upscale;
        std::cout<<img.feature.sizes();
        cv::Mat this_img = img.feature_mat;
        Size dsize = Size(2 * img.feature_mat.cols, 2 * img.feature_mat.rows);
        cv::resize(this_img, upscale, dsize, 0, 0, cv::INTER_CUBIC);
        cv::imshow("bicubicUpscale", upscale);
        cv::waitKey(0);
        sr_img = upscale.clone();
...
void MainWindow::on_pushButton_sr_clicked()
{
    if(ui->label_show->pixmap()!=nullptr){
        ...
        //cv::Mat original = tensor2Mat(feature);
